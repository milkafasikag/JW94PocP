...
def get_chat_rooms(request):
    ...
    for room in rooms:
        online_users = room.online.all()
        first_user = online_users.first() if online_users.exists() else None

        if first_user:
            room_list.append({
                'id': room.id,
                'name': room.name,
                'first_user_name': first_user.username  # Get the first name of the first online user
            })
...
def faqq(request):
    ...
    if request.user.is_authenticated:
        ...
        return render(request, 'demo-construction-careers.html',context)
    return render(request, 'demo-construction-careers.html', {"social":social, "clien":clien,"worke":worke,"testmon":testmon,"json_data": json_data_string, 'logstat':request.user.is_authenticated,'login':nope,'form': form, 'services': services,'register': CustomUserCreationForm()})


def blog(request):
    form = EmailForm()
    blogg = proj.Blog.objects.all().order_by('-id')
    clien = Client.objects.all().order_by('-id')
    worke = worker.objects.all().order_by('-id')
    testmon = testmone.objects.all().order_by('-id')
    social = socilamedia_company.objects.all().order_by('-id')
...
def project(request):
    ...
    if request.user.is_authenticated:
        ...
        return render(request, 'demo-construction-projects-5.html',context)
    return render(request, 'demo-construction-projects-5.html', {"social":social, "clien":clien,"worke":worke,"testmon":testmon,"json_data": json_data_string, 'logstat':request.user.is_authenticated,'login':nope,'form': form, 'blog': blogg,'register': CustomUserCreationForm()})
...
def services12(request):
    ...
    return render(request, 'demo-construction-services.html', {"social":social, "clien":clien,"worke":worke,"testmon":testmon,"json_data": json_data_string, 'logstat':request.user.is_authenticated,'login':nope,'form': form, 'item': services,'register': CustomUserCreationForm()})
...
def create_test(request):
    ...
    website = request.POST.get('website')
    image = request.FILES.get('image')
    client = testmone(name=name, description=website, image=image)
    client.save()
...
def create_galry(request):
    ...
    name = request.POST.get('name')
    image = request.FILES.get('image')
    client = galry(description=name,image=image)
    client.save()
